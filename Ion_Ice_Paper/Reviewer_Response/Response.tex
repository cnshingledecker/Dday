...
\subsubsection{Authors' Response}
\begin{responsebox}
\responselabel{} We thank the reviewer for this suggestion. To address this
point directly, we carried out a same-model ion-off control using the
manuscript-native Model~1 input deck. Rather than reverting to an earlier
ion-free network, we suppressed only the photoprocess channels yielding
charged products, while holding all neutral channels, trial frequencies, and
all other Model~1 parameters fixed. This isolates the role of the ionic
sector within the present model framework.

Quantitatively, the agreement with the \citet{gerakines_ultraviolet_1996}
\ce{O3} data deteriorates sharply when the charged-product channels are
muted. The unweighted root-mean-square deviation $\chi$ increases from 0.484
in the ion-enabled Model~1 calculation to 13.93 in the ion-off control, while
the peak modeled \ce{O3} abundance falls from 23.5\% to 3.24\% of the initial
\ce{O2}, corresponding to an 86\% reduction in the peak yield. At
representative intermediate fluences, where the observed ozone rise is most
diagnostic, the modeled \ce{O3}/\ce{O2}$_0$ ratio is likewise strongly
suppressed: at $F \approx 1.17\times10^{16}$ photons~cm$^{-2}$ it decreases
from 7.75\% to 0.16\%, at $F \approx 3.41\times10^{16}$ photons~cm$^{-2}$ from
15.70\% to 0.45\%, and at $F \approx 1.08\times10^{17}$ photons~cm$^{-2}$ from
22.58\% to 1.24\%. Thus, among the experimentally constrained neutral
abundances, \ce{O3} is the species most strongly affected by removal of the
ionic channels.

We have added this comparison to the revised manuscript together with a new
ion-on / ion-off overlay figure. We also now make explicit that this
same-model control is more informative than comparison to earlier ion-free
work such as \citet{mullikin_new_2021}, because it isolates the ionic
contribution without changing the rest of the network or parameter set. The
result does not imply that neutral chemistry cannot form ozone; indeed, the
route analysis still shows that neutral channels dominate some gross terminal
\ce{O3}-forming fluxes. Rather, it shows that within the present Model~1
framework the ionic sector is kinetically important for recovering both the
magnitude and fluence dependence of the observed \ce{O3} growth.
\end{responsebox}
