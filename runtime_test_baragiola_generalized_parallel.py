...
num_cores = 0
with open("experimental_data/num_cores.csv", "r") as num_cores_csv:
    reader = csv.reader(num_cores_csv, delimiter=",")
    for line in reader:
        for number in line:
            num_cores = int(number)
...
# Notes: core is the one that handles the generation and distribution of fitting factors and the collection of data.
